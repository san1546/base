...
layui.define(['table'], function (exports) {
  ...
  var $ = layui.$
    ...
    , tablePlug = {
      version: '0.1.0' // tablePlug的版本后面提交的时候会更新也好知道使用的是不是同一个版本
    }
    , tableIns = {}
    ...
    , function (event) {
      ...
      top.layer._indexTemp['selectInTable'] = top.layer.open({
        ...
        success: function (layero, index) {
          ...
          // } else {
          //   // debugger;
          //   form.render('select', titleElem.closest('.layui-form').attr('lay-filter'));
          //   // 处理ie浏览器下dl中html丢失的问题 后面需要调优的逻辑
          //   // dlClone.find('dd.layui-this').removeClass('layui-this');
          //   // dlClone.find('dd[lay-value="' + titleElem.closest('.layui-form-select').prev().val() + '"]')
          //   //   .addClass('layui-this');
          //   // dlClone.insertAfter(titleElem);
          // }
